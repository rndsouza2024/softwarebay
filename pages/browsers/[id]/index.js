...
const browserDetail = ({ browserItem }) => {
  ...
  const handleDownloadClick = () => {
    setShowTimer(true)
    setSeconds(10) // Example timer duration
  }
  ...
  useEffect(() => {
    ...
    const initializePlayer = () => {
      const videoIds = browserItem.videoitem
      const randomVideoId =
    ...
    initializePlayer()
  }, [playerReady, showTimer, browserItem])
  ...
  const softwareSchema = JSON.stringify({
    ...
    datePublished: browserItem.datePublished,
    dateModified: browserItem.dateModified,
    softwareVersion: browserItem.softwareVersion,
    url: browserItem.siteurl,
    operatingSystem: browserItem.operatingSystem,
    applicationCategory: browserItem.applicationCategory,
    image: browserItem.image,
    name: browserItem.name,
    interactionStatistic: browserItem.interactionStatistic,
    offers: {
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'browserItem',
        item: browserItem.baseurl
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: browserItem.name,
        item: browserItem.siteurl
      }
    ]
  })
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      {
        '@type': 'WebPage',
        '@id': `${browserItem.siteurl}#webpage`,
        url: browserItem.siteurl,
        name: `${browserItem.name} | Softwarebay`,
        datePublished: browserItem.datePublished,
        dateModified: browserItem.dateModified,
        isPartOf: {
          '@id': 'https://softwarebay.vercel.app#website'
      ...
      {
        '@type': 'Article',
        '@id': `${browserItem.siteurl}#article`,
        headline: `Download ${browserItem.name} | Softwarebay`,
        datePublished: browserItem.datePublished,
        dateModified: browserItem.dateModified,
        articleSection: 'browseral',
        author: {
          '@id': 'https://softwarebay.vercel.appauthor/browserItem/'
        },
        publisher: {
          '@id': 'https://gravatar.com/drtrailer2022/#person'
        },
        ...
        image: browserItem.image,
        name: `Download ${browserItem.name} | Softwarebay`,
        isPartOf: {
          '@id': `${browserItem.siteurl}#webpage`
        },
        inLanguage: 'en-US',
      ...
      {
        '@type': 'BlogPosting',
        '@id': `${browserItem.siteurl}#blogPost`,
        headline: `Download ${browserItem.name} | Softwarebay`,
        datePublished: browserItem.datePublished,
        dateModified: browserItem.dateModified,
        articleSection: 'browserItem',
        author: {
        ...
        image: browserItem.image,
        name: `Download ${browserItem.name} | Softwarebay`,
        '@id': `${browserItem.siteurl}#richSnippet`,
        isPartOf: {
          '@id': `${browserItem.siteurl}#webpage`
        },
        inLanguage: 'en-US',
  ...
  const newsArticleSchema = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    '@id': `${browserItem.siteurl}#webpage`, // Add a comma here
    name: browserItem.title,
    url: browserItem.siteurl,
    description: browserItem.news1,
    image: browserItem.image,
    datePublished: browserItem.startDate,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
        name: browserItem.title,
        urlTemplate: browserItem.siteurl
      }
    },
    locationCreated: {
      '@type': 'Place',
      name: browserItem.country
    },
    author: {
      '@type': 'Person',
      name: 'DrTrailer',
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title> Download {browserItem && browserItem.name} | Softwarebay</title>
        <link rel='canonical' href={browserItem && browserItem.siteurl} />
        ...
        <meta
          property='og:title'
          content={`${browserItem && browserItem.name} - Softwarebay`}
        />
        <meta
        ...
        <meta
          ...
          content='Explore. Discover. Download - Free Your Desired Software.'
        />
        <meta property='og:url' content={`${browserItem && browserItem.url}`} />
        <meta
          name='keywords'
          content={`${browserItem && browserItem.keywords}`}
        />
        <meta property='og:site_name' content='Softwarebay' />
        <meta property='og:type' content='article' />
        <meta
          property=' og:image:alt'
          content={`${browserItem && browserItem.group}`}
        />
        <meta name='mobile-web-app-capable' content='yes' />
        <meta property='article:section' content='Web Browser' />
        ...
        <meta
          property='og:image'
          content={`${browserItem && browserItem.backimage}`}
        />
          ...
          <div className='category-container'>
            ...
            >
              ...
              <p className='text-black text-bg font-semibold mt-2'>
                License: {browserItem.license}, Version: {browserItem.version}
              </p>
              <p className='text-black text-bg font-semibold mt-2'>
                Developers: {browserItem.developers}
              </p>
              <div
              ...
              >
                {!showTimer ? (
                  ...
                  <>
                    ...
                    ></div>
                    {seconds === 0 && (
                      <Link href={browserItem.downloadlink} target='_blank'>
                        <div
                          target='_blank'
                          className='bg-gradient-to-r from-amber-500 to-pink-500 text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:from-amber-600 hover:to-pink-600 transition duration-300'
                          style={{
              ...
              <p className='text-black text-bg font-semibold mt-2 text-3xl mb-2 items-center justify-center '>
                <strong> {browserItem.head1} </strong>
              </p>
              <Image
                src={browserItem.image1}
                alt={browserItem.name}
                width={1280}
                height={720}
                // priority
                objectFit='cover'
              ...
              <p className='text-black text-bg font-semibold mt-2 text-3xl mb-2 items-center justify-center '>
                <strong> {browserItem.head2} </strong>
              </p>
              <Image
                src={browserItem.image2}
                alt={browserItem.name}
                width={1280}
                height={720}
                // priority
              ...
              <p className='text-black text-bg font-semibold mt-2 text-3xl mb-2 items-center justify-center '>
                <strong> {browserItem.head3} </strong>
              </p>
              <Image
                src={browserItem.image3}
                alt={browserItem.name}
                width={1280}
                height={720}
                // priority
              ...
              <p className='text-black text-bg font-semibold mt-2 text-3xl mb-2 items-center justify-center '>
                <strong> {browserItem.head4} </strong>
              </p>
              <Image
                src={browserItem.image4}
                alt={browserItem.name}
                width={1280}
                height={720}
                // priority
...
export async function getStaticPaths () {
  const paths = browserData.map(item => ({
    params: { id: item.id }
  }))
...
export async function getStaticProps ({ params }) {
  const browserItem = browserData.find(item => item.id === params.id)
  return { props: { browserItem } }
}
