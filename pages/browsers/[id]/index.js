...
import browserssData from '../../../public/browsers.json'
...
const browsersDetail = ({ browsers }) => {
  ...
  const softwareSchema = JSON.stringify({
    ...
    datePublished: browsers.datePublished,
    dateModified: browsers.dateModified,
    softwareVersion: browsers.softwareVersion,
    url: browsers.siteurl,
    operatingSystem: browsers.operatingSystem,
    applicationCategory: browsers.applicationCategory,
    image: browsers.image,
    name: browsers.name,
    interactionStatistic: browsers.interactionStatistic,
    offers: {
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'browsers',
        item: browsers.baseurl,
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: browsers.name,
        item: browsers.siteurl,
      }
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      {
        '@type': 'WebPage',
        '@id': `/${browsers['browsers.watch']}#webpage`,
        url: `/${browsers['browsers.watch']}`,
        name: `${browsers.name} | Softwarebay`,
      ...
      {
        '@type': 'Person',
        '@id': 'https://softwarebay.vercel.appauthor/browsers/',
        name: 'Dr Trailer',
        url: 'https://softwarebay.vercel.appauthor/browsers/',
        image: {
          '@type': 'ImageObject',
      ...
      {
        '@type': 'Article',
        '@id': `/${browsers['browsers.watch']}#article`,
        headline: `Watch ${browsers.name} | Softwarebay`,
        datePublished: '2024-01-13T13:00:00+00:00',
        dateModified: '2024-01-13T13:13:00+00:00',
        ...
        image: browsers.image,
        name: `Watch ${browsers.name} | Softwarebay`,
        isPartOf: {
          '@id': `/${browsers['browsers.watch']}#webpage`
        },
        inLanguage: 'en-US',
      ...
      {
        '@type': 'BlogPosting',
        '@id': `/${browsers['browsers.watch']}#blogPost`,
        headline: `Watch ${browsers.name} | Softwarebay`,
        datePublished: '2024-01-13T13:00:00+00:00',
        dateModified: '2024-01-13T13:13:00+00:00',
        articleSection: 'browsers',
        ...
        image: browsers.image,
        name: `Watch ${browsers.name} | Softwarebay`,
        '@id': `/${browsers['browsers.watch']}#richSnippet`,
        isPartOf: {
          '@id': `/${browsers['browsers.watch']}#webpage`
        },
        inLanguage: 'en-US',
  ...
  const newsArticleSchema = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    '@id': `/${browsers['browsers.watch']}`,
    name: browsers.title,
    url: `/${browsers['browsers.watch']}`,
    description: browsers.synopsis,
    image: browsers.image,
    datePublished: browsers.startDate,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
        name: browsers.title,
        urlTemplate: `${browsers['url']}`
      }
    },
    locationCreated: {
      '@type': 'Place',
      name: browsers.country
    },
    author: {
      '@type': 'Person',
      name: 'DrTrailer',
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title> Watch {browsers && browsers.name} | Softwarebay</title>
        <link rel='canonical' href={browsers && browsers.siteurl} />
        ...
        <meta
          property='og:title'
          content={`${browsers && browsers.name} - Softwarebay`}
        />
        <meta
        ...
        <meta
          ...
          content='Explore. Discover. Download - Free Your Desired Software.'
        />
        <meta property='og:url' content={`${browsers && browsers.url}`} />
        <meta name='keywords' content={`${browsers && browsers.keywords}`} />
        <meta property='og:site_name' content='Softwarebay' />
        <meta property='og:type' content='article' />
        <meta
          property=' og:image:alt'
          content={`${browsers && browsers.group}`}
        />
        ...
        <meta
          property='og:image'
          content={`${browsers && browsers.backimage}`}
        />
      ...
      >
        ...
        >
          ...
          >
            ...
            <button className='border border-black p-2 m-1 hover:bg-orange-100'>
              <li id='menu-item-11606' className='menu-browsers'>
                <a
          ...
          <div className='category-container'>
            ...
            >
              ...
              <p className='text-black text-bg font-semibold mt-2'>
                License: {browsers.license}, Version: {browsers.version}
              </p>
              <p className='text-black text-bg font-semibold mt-2'>
                Developers: {browsers.developers}
              </p>
              {/*               
              {browsers.synopsis.split('\n\n').map((paragraph, idx) => (
                <p
                  key={idx}
              ...
              >
                {!showTimer ? (
                  ...
                  <>
                    ...
                    {seconds === 0 && (
                      <Link href={browsers.downloadlink} target='_blank'>
                        <div
                          target='_blank'
                          className='bg-gradient-to-r from-amber-500 to-pink-500 text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:from-amber-600 hover:to-pink-600 transition duration-300'
                          style={{
...
export async function getStaticPaths () {
  const paths = browserssData.map(browsers => ({
    params: { id: browsers.id } // Assuming each browsers object has an "id" field
  }))
  return { paths, fallback: false }
}

export async function getStaticProps ({ params }) {
  // Fetch the browsers data based on the provided id
  const browsers = browserssData.find(m => m.id === params.id)
