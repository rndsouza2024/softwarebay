...
import educationsData from '../../../public/education.json'
...
const educationDetail = ({ education }) => {
  ...
  const softwareSchema = JSON.stringify({
    ...
    datePublished: education.datePublished,
    dateModified: education.dateModified,
    softwareVersion: education.softwareVersion,
    url: education.siteurl,
    operatingSystem: education.operatingSystem,
    applicationCategory: education.applicationCategory,
    image: education.image,
    name: education.name,
    interactionStatistic: education.interactionStatistic,
    offers: {
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'Education',
        item: education.baseurl,
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: education.name,
        item: education.siteurl,
      }
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      {
        '@type': 'WebPage',
        '@id': `/${education['education.watch']}#webpage`,
        url: `/${education['education.watch']}`,
        name: `${education.name} | Softwarebay`,
      ...
      {
        '@type': 'Person',
        '@id': 'https://softwarebay.vercel.appauthor/education/',
        name: 'Dr Trailer',
        url: 'https://softwarebay.vercel.appauthor/education/',
        image: {
          '@type': 'ImageObject',
      ...
      {
        '@type': 'Article',
        '@id': `/${education['education.watch']}#article`,
        headline: `Watch ${education.name} | Softwarebay`,
        datePublished: '2024-01-13T13:00:00+00:00',
        dateModified: '2024-01-13T13:13:00+00:00',
        ...
        image: education.image,
        name: `Watch ${education.name} | Softwarebay`,
        isPartOf: {
          '@id': `/${education['education.watch']}#webpage`
        },
        inLanguage: 'en-US',
      ...
      {
        '@type': 'BlogPosting',
        '@id': `/${education['education.watch']}#blogPost`,
        headline: `Watch ${education.name} | Softwarebay`,
        datePublished: '2024-01-13T13:00:00+00:00',
        dateModified: '2024-01-13T13:13:00+00:00',
        articleSection: 'education',
        ...
        image: education.image,
        name: `Watch ${education.name} | Softwarebay`,
        '@id': `/${education['education.watch']}#richSnippet`,
        isPartOf: {
          '@id': `/${education['education.watch']}#webpage`
        },
        inLanguage: 'en-US',
  ...
  const newsArticleSchema = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    '@id': `/${education['education.watch']}`,
    name: education.title,
    url: `/${education['education.watch']}`,
    description: education.synopsis,
    image: education.image,
    datePublished: education.startDate,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
        name: education.title,
        urlTemplate: `${education['url']}`
      }
    },
    locationCreated: {
      '@type': 'Place',
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title> Watch {education && education.name} | Softwarebay</title>
        <link rel='canonical' href={education && education.siteurl} />
        ...
        <meta
          property='og:title'
          content={`${education && education.name} - Softwarebay`}
        />
        <meta
        ...
        <meta
          ...
          content='Explore. Discover. Download - Free Your Desired Software.'
        />
        <meta property='og:url' content={`${education && education.url}`} />
        <meta name='keywords' content={`${education && education.keywords}`} />
        <meta property='og:site_name' content='Softwarebay' />
        <meta property='og:type' content='article' />
        <meta
          property=' og:image:alt'
          content={`${education && education.group}`}
        />
        ...
        <meta
          property='og:image'
          content={`${education && education.backimage}`}
        />
          ...
          <div className='category-container'>
            ...
            >
              ...
              <p className='text-black text-bg font-semibold mt-2'>
                License: {education.license}, Version: {education.version}
              </p>
              <p className='text-black text-bg font-semibold mt-2'>
                Developers: {education.developers}
              </p>
              {/*               
              {education.synopsis.split('\n\n').map((paragraph, idx) => (
                <p
                  key={idx}
              ...
              >
                {!showTimer ? (
                  ...
                  <>
                    ...
                    {seconds === 0 && (
                      <Link href={education.downloadlink} target='_blank'>
                        <div
                          target='_blank'
                          className='bg-gradient-to-r from-amber-500 to-pink-500 text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:from-amber-600 hover:to-pink-600 transition duration-300'
                          style={{
...
export async function getStaticPaths () {
  const paths = educationsData.map(education => ({
    params: { id: education.id } // Assuming each education object has an "id" field
  }))
  return { paths, fallback: false }
}

export async function getStaticProps ({ params }) {
  // Fetch the education data based on the provided id
  const education = educationsData.find(m => m.id === params.id)
